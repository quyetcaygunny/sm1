...
class Brand extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _construct(){
        $this->_init('sumup_brand','brand_id');
    }
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['sumup_brand_store' => $this->getTable('sumup_brand_store')],
                $this->getMainTable() . '.brand_id = sumup_brand_store.brand_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'sumup_brand_store.store_id IN (?)',
                $storeIds
            )->order(
                'sumup_brand_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('sumup_brand_store')],
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('sumup_brand_store'), $condition);

        $condition = ['brand_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('sumup_brand_products'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('sumup_brand_store');
        ...
        if(null !== ($object->getData('products'))){
            $table = $this->getTable('sumup_brand_products');
    ...
    public function getBrandIdByName($brand_name = '') {
        if($brand_name) {
            $brand_id = null;
            $table = $this->getTable('sumup_brand');
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($id = $object->getId()) {
            ...
            $select = $connection->select()
                ->from($this->getTable('sumup_brand_products'))
                ->where(
                    'brand_id = '.(int)$id
                );
    ...
    public function lookupStoreIds($brandId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('sumup_brand_store'),
            'store_id'
        )
            ->where(
                'brand_id = ?',
